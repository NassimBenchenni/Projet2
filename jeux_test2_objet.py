...
class Space_invaders:
    def __init__(self, score, compteur_ennemie, text_potion, vies, niveau):
        self.score = score
        self.ennemie = compteur_ennemie
        self.boost = text_potion
        self.vies = vies
        self.niveau = niveau
    ...
    def mise_a_jour():
        conn = sqlite3.connect('data.db')
        cur = conn.cursor()
        info = {"Vie":self.vies, "Ennemies":self.ennemie, "Score": self.score, "Potion": self.boost}
    ...
    def vaisseau_deplacement(self, x, y, speed):
        """Déplacement avec les touches de directions, en fonction de la vitesse et du niveau"""
        global random_target_position, boost_vitesse, boost_text_counter
        ...
        if self.niveau > 0 and self.niveau % 2 == 0:  # Changer cette condition selon le niveau que vous ciblez
    ...
    def incrementer_compteur(self, compteur_niveau):

        self.niveau += 1
        compteur_niveau += 5
        self.compteur_ennemie = compteur_niveau
        return niveau
        ...
        for ennemi in ennemis_liste:
            for tir in tirs_liste:
                if ennemi[0] <= tir[0]+1 and ennemi[0]+8 >= tir[0] and ennemi[1]+8 >= tir[1]:
                    ennemis_liste.remove(ennemi)
                    self.compteur_ennemie -= 1
                    if self.compteur_ennemie == 0:
    ...
    def draw():
        ...
        if vies > 0:
            ...
            pyxel.text(85,5, 'Ennemies:' + str(self.compteur_ennemie), 8)
