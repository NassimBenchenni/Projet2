...
def mise_a_jour():
    ...
    cur.execute("UPDATE info SET Vie = :vies, Ennemie = :Ennemies, Score = :Score, Potion = :Potion", info)
    conn.commit()
...
"""def repr_vie():
    res = cur.execute("SELECT Vie FROM info")
    Vie2 = res.fetchone()
    print(Vie2)
    return Vie2[0]
...
def repr_potion():
    res = cur.execute("SELECT Potion FROM info")
    Potion2 = res.fetchall()
    conn.commit()
    conn.close()    
    return Potion2

def repr_Ennemie():
    res = cur.execute("SELECT Ennemie FROM info")
    Ennemie2 = res.fetchall()
    conn.commit()
    conn.close()    
    return Ennemie2"""
...
def draw():
    ...
    if vies > 0:
        ...
        # affichage des vies            
        pyxel.text(5,5, 'VIES:'+ str(repr_vie()), 7)
        ...
        # Afficher le score
        pyxel.text(5, 115, 'Score:' + str(repr_score()), 9)
